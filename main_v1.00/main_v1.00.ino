...
const long special_date = 1403913000;
...
void writeMatrix_randError(){
  if(millis() < SHUTDOWN){
    ...
    for(int i = 0; i < 8; i++){
      unsigned int value = matrix_empty[i];
      digitalWrite(CS_DOT, LOW);
      SPI.transfer(i + 1);
      SPI.transfer(value);
      digitalWrite(CS_DOT, HIGH);
...
long getYearSince(){
  ...
  return (long)((unixNow.unixtime() - special_date) / 86400);
}
...
void loop() {
  ...
  if (currentMillis - segment_previousMillis >= SEGMENT_INTERVAL && currentMillis < SHUTDOWN) {
    segment_previousMillis = currentMillis;
    writeDigits(getYearSince());
  }
  
  if (currentMillis - matrix_previousMillis >= MATRIX_INTERVAL && currentMillis < SHUTDOWN) {
